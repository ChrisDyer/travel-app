...
## Phase 5 — Timezone-correct feed (absolute instants) — 2026-08-03

**Status:** complete-with-deviations — code, migration and docs done and verified locally;
production deploy pending (migrate → backfill → deploy, in that order).

**Why this phase exists:** Phases 1–4 emitted **floating** datetimes (`DTSTART:20260808T114500`,
no `TZID`, no `Z`). RFC 5545 calls that "local wherever viewed" and `00-overview.md` treated it as
load-bearing — `ics.ts` literally said "Do not add it" about `X-WR-TIMEZONE`. **Google Calendar
does not implement floating times for subscribed feeds**; it normalises them to UTC. Chris reported
every time "way off… on Greenwich time" within hours of subscribing, and the production feed
confirmed it: 48 timed events, all floating. An 11:45 Chicago departure rendered as 06:45.

The design premise was wrong, not the implementation of it. This phase replaces the premise.

**What was built/done:**

- **`src/lib/calendar/timezone.ts`** (import-free) — `wallTimeToInstant` converts a wall time in an
  IANA zone to an absolute instant using only `Intl`. It **brackets and validates** rather than
  guessing-then-correcting-once, because the naive form walks *backwards* through a spring-forward
  gap. `isValidTimeZone` try/catches the `DateTimeFormat` constructor and must never become a
  `Intl.supportedValuesOf` lookup — those disagree, and the geocoder returns the modern names.
  `extractIata` is deliberately conservative (no bare 3-letter tokens inside longer text).
- **`src/lib/calendar/airport-timezones.ts`** — generated by `tools/build-airport-timezones.mjs`
  from OurAirports coordinates through `tz-lookup` (a new devDependency), restricted to the IATA
  codes already in `public/airports.json`. 4,560 of 4,564 codes, 372 zones, 61 KB. Offline by
  design: a Google poll must make zero network calls.
- **Migration `011_location_timezones`** — `trips.timezone` (user override), plus
  `trips.resolved_timezone` and `trip_legs.resolved_timezone` (geocoder cache). Two columns on
  `trips` on purpose: the derived one is cleared with the geocode on a destination edit, the
  override is not.
- **`src/lib/geocode.ts`** now keeps the IANA `timezone` Open-Meteo has always returned and this
  code had been casting away. No extra request, no extra latency.
- **Emission** — `dtProperty` takes an `EventEndpoint` and emits either a `Z` instant or an all-day
  `VALUE=DATE`. **The floating branch is deleted**, which makes the regression unrepresentable
  rather than merely discouraged. New `X-ZO-TZ` property records the source zone(s).
- **Resolution chain in `items.ts`**: endpoint's airport (flights only) → covering leg → trip
  override → trip geocode → the other endpoint's zone. Flights resolve each end separately;
  `flightReturn` swaps them.
- **Unresolved → all-day with the time in the title**, `X-ZO-TZ:unresolved`, UID unchanged. Count
  surfaced on the Settings card. Per-trip **Timezone** select added to the trip edit form.
- **`tools/backfill-timezones.mjs`** — idempotent, never bumps `updated_at`, `--dry-run` supported.

**Decision: UTC `Z`, not `TZID` + `VTIMEZONE`.** Chiefly because a flight's start and end are in
*different* zones (`flightReturn` swaps them; transit and one-way rentals do the same) and `Z`
handles that with no extra machinery, where `TZID` needs two `VTIMEZONE` components per flight and
correct client handling of a per-property `TZID`. Also: any `VTIMEZONE` generator must pick a year
window, and a window that is a function of request time or of the data would break the
byte-identical-refetch contract. Google renders both identically.

**Deviations from spec (and why):**

- **`X-WR-TIMEZONE` stays absent and its test survives** — but the *reasoning* is inverted and both
  comments now say so. With everything in `Z` there is no floating value for it to pin, and Google
  reads it as the calendar's display zone, which would override each subscriber's own.
- The plan's §5.2 "best-effort inline geocode on destination PATCH" is implemented, and its
  departure from "only `/api/map` fills these" is commented at the call site.

**A real problem this surfaced in live data:**

The Washington trip's destination is the string `"Washington"`, which the geocoder resolves —
confidently and wrongly — to **Washington DC (`America/New_York`)**, not Seattle. Its hotel
check-in was stamped 16:00 Eastern = 13:00 Pacific, three hours out. This is pre-existing, not new:
the weather panel has been showing Washington D.C. for this trip all along. Nothing can flag it
automatically, because the geocoder is confident rather than silent. The per-trip **Timezone**
override is the fix and was verified end to end on this exact trip. Flights were unaffected
throughout — IATA codes are unambiguous.

**Known gaps / follow-ups:**

- **Not yet deployed.** Order matters: migrate → run the backfill → confirm
  `SELECT COUNT(*) FROM trips WHERE COALESCE(timezone, resolved_timezone) IS NULL` is 0 → deploy.
  Deploying first would demote a large share of events to all-day and flip them back hours later,
  visible churn in both subscribers' calendars.
- **Transit and rental cars do not resolve per-endpoint zones.** `from_location`/`to_location` are
  free text with no code to key on, so both ends take the leg/trip zone. A Eurostar London→Paris
  would have its arrival an hour out. Flights, the common cross-zone case, are handled.
- Byte-stability now holds only *within a deploy*: a Node upgrade shipping new tzdata can change a
  future instant with no edit. That is correct behaviour — the real instant did change.
- `items.ts` still has no unit test (it imports `@/db`); the resolution chain is covered by the
  export diff below and the pure modules carry the tricky logic.

**Verification evidence:**

1. `node --test src/lib/calendar/*.test.mjs` — **128 pass, 0 fail** (up from 98; +23 timezone,
   +11 airport table, and the rewritten ICS suite).
2. Every DST edge case asserted on the exact instant *and* the resolution class: spring-forward gap
   shifts **forward** (02:30 → 03:30 EDT, not backwards to 01:30 EST), fall-back overlap takes the
   earlier instant, Lord Howe's 30-minute shift, Kathmandu +05:45, Chatham +12:45.
3. A test asserts `isValidTimeZone('Asia/Kolkata')` is true *and* that it is absent from
   `Intl.supportedValuesOf` — the guard against "simplifying" the validator into a set lookup.
4. My own airport test caught a real contract violation: `AIRPORT_TZ_INDEX['toString']` resolved to
   the inherited `Object.prototype` function, so the lookup returned `undefined` typed as `string`.
   Fixed in the generator with a `typeof` guard.
5. **UIDs unchanged**: 8 before, 8 after, none added, none removed — nothing orphans in a
   subscriber's calendar.
6. **All-day lines byte-identical** before and after: the change is confined to timed events.
7. Two consecutive fetches byte-identical.
8. A cross-zone flight resolves each end independently:
   `DTSTART:20260808T164500Z` / `DTEND:20260808T211900Z` with
   `X-ZO-TZ:America/Chicago/America/Los_Angeles` — 11:45 Chicago to 14:19 Seattle.
9. Override verified live: setting the Washington trip to `America/Los_Angeles` moved the Lotte
   check-in from `20:00Z` (13:00 Pacific, wrong) to `23:00Z` (16:00 Pacific, right), and the
   Mariners game to 13:10 Pacific. `unresolvedTimezones` went to 0.
